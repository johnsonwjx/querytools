...
public class BackgroupPicEditor extends JPanel implements ValueEditor {
	public BackgroupPicEditor() {
		...
		JButton button_4 = new JButton("\u63D2\u5165\u65E5\u671F\u6807\u5FD7");
	...
	private void init() {
		model = new DefaultTableModel(new String[] { "条件", "选择图片" }, 0);
		...
		ButtonCellEditor cellEditor = new ButtonCellEditor(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String pic = (String) model.getValueAt(row, column);
				JFileChooser chooser = XMLUtils.getFileChooser();
				chooser.setFileFilter(new FileNameExtensionFilter("pic file", "jpg", "gif", "png", "bmp", "jpeg"));
				if (!CommonUtils.isStrEmpty(pic)) {
					chooser.setSelectedFile(new File(pic));
				}
				int flag = chooser.showOpenDialog(dialog);
				if (flag == JFileChooser.APPROVE_OPTION) {
					File file = chooser.getSelectedFile();
					if (file == null) {
						table.setValueAt("", row, column);
					} else {
						table.setValueAt(file.getName(), row, column);
					}
	...
	private void save() {
		try {
			...
			if (model.getRowCount() <= 0) {
				...
				root.addElement("columns").setText("");
				...
				if (model.getRowCount() > 0) {
					for (int i = 0; i < model.getRowCount(); i++) {
						...
						if (CommonUtils.isStrEmpty(condi) || CommonUtils.isStrEmpty(color)) {
							GUIUtils.showMsg(dialog, "必须同时设置图片和条件");
	...
	private void initData(String value) {
		...
		try {
			...
			for (Element item : items) {
				...
				rowData[1] = item.elementText("color");
	...
	public void edit(Window owner, Map<String, String> props) {
		...
		initData(value);
		dialog = GUIUtils.getDialog(owner, "表格背景图片设置", this);
